function [h, Sh, index, R] = build4DOFbnetState(bnet, index, x, P, y, o0, p0, do0)
...
for i = 1:4
    ...
    if i == 3 || i == 4
        ...
        if i == 3
             ...
             W(3, 7) = 1;
             W = [W, Ri'];
             bnet.CPD{index.o(i)}  = gaussian_CPD(bnet, index.o(i),  'mean', [0 0 0]', 'cov', diag([1e-2 1e-2 1e-2]), 'weights', W);
        else
             W = zeros(3,8);
             W(3, 8) = 1;
             W = [W, Ri'];
...
%         end
        ...
        if i == 1
            ...
            bnet.CPD{index.o(i)}  = gaussian_CPD(bnet, index.o(i),  'mean', [0 0 0], 'cov', diag([1e-1 1e-1 1e-1]));            
        else
            W = Ci(indC:indC+2, colum.o(j_prev));            
            b = bi(indC:indC+2) ;
            % bnet.CPD{index.o(i)}  = gaussian_CPD(bnet, index.o(i),  'mean', b, 'cov', diag([1e2 1e2 1e-6]), 'weights', W);
            bnet.CPD{index.o(i)}  = gaussian_CPD(bnet, index.o(i),  'mean', [0 0 0], 'cov', diag([1e-1 1e-1 1e-1]), 'weights', eye(d));
        end
        indC = indC+3;
        ...
        %% do(:,i) = Ri'*do_prev;
        if i == 1
            b = Ci(indC:indC+2, colum.do0)*do_prev + bi(indC:indC+2) ;
        ...
        if i == 1
            b  = Ci(indC:indC+2, colum.p0)*p_prev + bi(indC:indC+2) ;
            W1 = Ci(indC:indC+2, colum.d2t(i));
            W2 = Ci(indC:indC+2, colum.do(j));
...
evidence{index.mo(4)}    = [0 0 0]';%y(colum.o(10:12));
