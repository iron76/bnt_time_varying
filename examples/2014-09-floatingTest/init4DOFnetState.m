function [bnet, index, labels] = init4DOFnetState(n, k, d)
...
s_x    = 8;                s_mx  = 2;
% nodes dimesions
ns = [s_p s_do s_d2q s_c s_p s_do s_d2q s_c s_f s_u s_fbb s_ubb s_fee s_uee s_f s_u s_o s_o s_x s_mx]; % all 3 dimensional vector for all but the d2q
...
index.x     = i_x;       index.mx  = m_x;

n_nodes = length(new_ns);
labels  = cell(n_nodes,1);

for i = 1: length(m_x)
    labels{m_x(i)} = strcat('mx', num2str(i));
end

for i = 1: length(m_o)
    labels{m_o(i)} = strcat('mo', num2str(i));
end

for i = 1: length(m_u)
    labels{m_u(i)} = strcat('mu', num2str(i));
end

for i = 1: length(m_f)
    labels{m_f(i)} = strcat('mf', num2str(i));
end

for i = 1: length(i_uee)
    labels{i_uee(i)} = strcat('ue', num2str(i));
end

for i = 1: length(i_fee)
    labels{i_fee(i)} = strcat('fe', num2str(i));
end

for i = 1: length(i_ubb)
    labels{i_ubb(i)} = strcat('ub', num2str(i));
end

for i = 1: length(i_fbb)
    labels{i_fbb(i)} = strcat('fb', num2str(i));
end

for i = 1: length(i_x)
    labels{i_x(i)} = strcat('x', num2str(i));
end
for i = 1: length(i_o)
    labels{i_o(i)} = strcat('o', num2str(i));
end
for i = 1: length(i_u)
    labels{i_u(i)} = strcat('u', num2str(i));
end
for i = 1: length(i_f)
    labels{i_f(i)} = strcat('f', num2str(i));
end
for i = 1: length(m_c)
    labels{m_c(i)} = strcat('mc', num2str(i));
end
for i = 1: length(m_d2q)
    labels{m_d2q(i)} = strcat('md2q', num2str(i));
end
for i = 1: length(i_c)
    labels{i_c(i)} = strcat('c', num2str(i));
end
for i = 1: length(i_d2q)
    labels{i_d2q(i)} = strcat('d2q', num2str(i));
end
for i = 1: length(m_do)
    labels{m_do(i)} = strcat('mdo', num2str(i));
end

for i = 1: length(i_do)
    labels{i_do(i)} = strcat('do', num2str(i));
end

for i = 1: length(i_p)
    labels{i_p(i)} = strcat('p', num2str(i));
end

for i = 1: length(m_p)
    labels{m_p(i)} = strcat('mp', num2str(i));
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
bnet.CPD{index.fbb}    = gaussian_CPD(bnet, index.fbb,     'mean', zeros(d,1), 'cov', Sfbb,'clamp_mean', 1, 'clamp_weights', 1, 'clamp_cov', 1);
bnet.CPD{index.ubb}    = gaussian_CPD(bnet, index.ubb,     'mean', zeros(d,1), 'cov', Subb,'clamp_mean', 1, 'clamp_weights', 1, 'clamp_cov', 1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
bnet.CPD{index.fee}    = gaussian_CPD(bnet, index.fee,     'mean', zeros(d,1), 'cov', Sfee,'clamp_mean', 1, 'clamp_weights', 1, 'clamp_cov', 1);
bnet.CPD{index.uee}    = gaussian_CPD(bnet, index.uee,     'mean', zeros(d,1), 'cov', Suee,'clamp_mean', 1, 'clamp_weights', 1, 'clamp_cov', 1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
bnet.CPD{index.mx}     = gaussian_CPD(bnet, index.mx ,     'mean', zeros(2,1), 'cov', Smx,'clamp_mean', 1, 'clamp_weights', 1,                                     'clamp_cov', 1, 'weights', Wx);
