...
[bnet0, index] = init4DOFnetState(n, k, d);

x = [0 0 0 0, 0 0 0 0]';
P = diag([1e6 1e6 1e6 1e6, 1e6 1e6 1e6 1e6]);
for j = 1 : length(time)
...
%     
    ...
    [h, Sh, indexH, R]    = build4DOFbnetState(bnet{j}, index, x, P, o0, p0, do0);
    ...
    xHat = h{indexH.x};
    PHat = Sh{indexH.x};
    legRightdqHat(1:4,j)   = [0   0   xHat   0];
    legRightdqSigma(1:4,j) = [0 0 PHat 0];
...
% est_ax = axis;
% subplot(324)
% plotWithSigma(time, legRight_dq(:,[1])*pi/180, repmat([P(7,7)], length(legRight_dq(:,[1 4])), 1), '')
% axis(est_ax);
% subplot(325)
% plotWithSigma(time, legRightdqHat(4,:)', legRightdqSigma(4,:)', '')
% est_ax = axis;
% subplot(326)
% plotWithSigma(time, legRight_dq(:,[4])*pi/180, repmat([P(8,8)], length(legRight_dq(:,[1 4])), 1), '')
% axis(est_ax);
% 
% 
% figure
...
% est_ax = axis;
% subplot(324)
% plotWithSigma(time, legRight_q(:,[1])*pi/180, repmat([P(3,3)], length(legRight_q(:,[1 4])), 1), '')
% axis(est_ax);
% axis(est_ax);
% subplot(325)
...
% est_ax = axis;
% subplot(326)
% plotWithSigma(time, legRight_q(:,[4])*pi/180, repmat([P(4,4)], length(legRight_q(:,[1 4])), 1), '')
% axis(est_ax);
