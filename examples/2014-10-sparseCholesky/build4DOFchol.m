function [h, Sh, Ro, avgTime, S1] = build4DOFchol(index, y, q, dq, o0, do0, p0,j, S1)
...
if j == 1 
    As = sparse(A);
    W = (As'*inv(sparse(P))*As);
    [L,p,S1] = chol(W, 'lower');    % S1'*W*S1 = L*L'
    % PWinv1 = S1*inv_chol(L)*S1';
    L1 = S1*L;
else
    As = sparse(A);
    W = (As'*inv(sparse(P))*As);
    L = chol(S1'*W*S1, 'lower');    % S1'*W*S1 = L*L'
    % PWinv1 = S1*inv_chol(L)*S1';
    L1 = S1*L;
end


Exy = -(L1'\(L1\A'))*inv1(P)*b + (L1'\(L1\S'))*inv1(R+S*(L1'\(L1\S')))*(y+S*(L1'\(L1\A'))*inv1(P)*b);
Sxy = inv1(A'*inv1(P)*A + S'*inv1(R)*S);
avgTime = toc;
