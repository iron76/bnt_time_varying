function  [d, Sd] = mID( model, q, qd , Yc , y)
...
Sv_inv = full(sparse(idSv_inv, jdSv_inv, dSv_inv));
% Sy_inv = eye(7*NB) ./sUknown;
Sy_inv = full(sparse(idSw_inv, jdSw_inv, dSw_inv));
% Sw_inv = eye(my)   ./sMeas;
Sw_inv = full(sparse(idSy_inv, jdSy_inv, dSy_inv));
% Dy_new = Dx\Dy;
% Sv     = inv(Sv_inv);
% Sy     = inv(Sy_inv);
% Sw     = inv(Sw_inv);
