...
for i = 1:4
    ...
    if i == 3 || i == 4
        ...
        bnet.CPD{index.o(i)}  = gaussian_CPD(bnet, index.o(i),  'mean', b, 'cov', eye(d), 'weights', W);
        indC = indC+3;
...
%         end
        ...
        if i ~= 1
            ...
            b = bi(indC:indC+2) - db(indC:indC+2);
            W = commuteOnOrder2(W1, W2, index.o(i-1), index.x);
            bnet.CPD{index.o(i)}  = gaussian_CPD(bnet, index.o(i),  'mean', b, 'cov', 1e-3.*eye(d), 'weights', W);
        else
            W = dC(indC:indC+2, :);
            b = bi(indC:indC+2) - db(indC:indC+2);
            bnet.CPD{index.o(i)}  = gaussian_CPD(bnet, index.o(i),  'mean', b, 'cov', 1e-3.*eye(d), 'weights', W);
