function  [tau, a, fB, f, net, Sf] = nID( model, q, qd, qdd, f_ext, y, net)
...
for i = 1:model.NB
  if model.parent(i) == 0
    ...
    W = cell2mat(Wa);
    net.bnet.CPD{ai} = gaussian_CPD(net.bnet, ai, 'mean', Xup{i}*(-a_grav), 'cov', sModel.*eye(nai), 'weights', W);      
  else
    ...
    W = cell2mat(Wa);
    net.bnet.CPD{ai} = gaussian_CPD(net.bnet, ai, 'mean', crm(v{i})*vJ, 'cov', sModel.*eye(nai), 'weights', W);
  end
  ...
  W = cell2mat(Wa);
  net.bnet.CPD{fBi} = gaussian_CPD(net.bnet, fBi, 'mean', crf(v{i})*model.I{i}*v{i}, 'cov', sModel.*eye(nfBi), 'weights', W);    
end
...
for i = model.NB:-1:1
  ...
  net.bnet.CPD{fi} = gaussian_CPD(net.bnet, fi, 'mean', zeros(nfi,1), 'cov', sModel.*eye(nfBi), 'weights', W);    
  ...
  W = cell2mat(Wa);
  net.bnet.CPD{taui} = gaussian_CPD(net.bnet, taui, 'mean', zeros(ntaui, 1), 'cov', sModel.*eye(ntaui), 'weights', W);    
  
  % fxi
  fxi  = net.nodes.index{i}(5);
  nfxi = net.nodes.sizes{i,1}(5);
  net.bnet.CPD{fxi} = gaussian_CPD(net.bnet, fxi, 'mean', zeros(nfxi, 1), 'cov', sUknown.*eye(nfxi));  
  
  % d2qi
  d2qi  = net.nodes.index{i}(6);
  nd2qi = net.nodes.sizes{i,1}(6);
  net.bnet.CPD{d2qi} = gaussian_CPD(net.bnet, d2qi, 'mean', zeros(nd2qi, 1), 'cov', sUknown.*eye(nd2qi));  
...
for i = 1 : ny
  ...
  net.bnet.CPD{yi} = gaussian_CPD(net.bnet, yi, 'mean', zeros(nyi, 1), 'cov', sMeas.*eye(nyi), 'weights', y.W{i,1});    
end
