...
i_obs    = [index.md2t(3) index.md2t(4) index.mf(2)  index.mu(2)...  
            index.mf(5)  index.mu(5)   index.mdo(4)  index.mp(4)];

i_cov_learn    = [index.md2t(3) index.md2t(4) index.mf(2)  index.mu(2)...  
                  index.mf(5)  index.mu(5)   index.mdo(4)  index.mp(4)];
...
for i=1:nsamples
    bnet0std{i} = insertStandardization(bnet{i}, i_cov_learn, M, S, covPriorWeight);
...
bnetHat = learn_params_em_modified(engineStd, samplesStd, 3, 1e-3);
bnetHat = removeStandardization(bnetHat{nsamples}, i_cov_learn, M, S, covPriorWeight);
...
hat_do4 = struct(bnetHat.CPD{index.mdo(4)});
Sdo4 = hat_do4.cov;
