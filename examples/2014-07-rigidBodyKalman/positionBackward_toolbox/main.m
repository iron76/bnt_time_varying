...
Q                = diag([ones(6,1).*dt*100; ones(6,1)*10; ones(4,1).*dt*10;]);

model.u   = 0;
model.v   = 0;

xh        = rand(size(model.x0')).*100 - 50 ;
Ph        = eye(n)*100; 
...
for i = length(t):-1:1
    ...
    [xh, Ph] =  ekf_predict1(xh, Ph, db_dx_func, Q, b_func, [], model);
