function plotResults(x, Xhat, P, t)

figure(1)
subplot(311)
shadedErrorBar(t,Xhat(:,1),squeeze(2*sqrt(P(1,1,:)))','b');
hold on
plot(t, x(:,1), '--b')

subplot(312)
shadedErrorBar(t,Xhat(:,2),squeeze(2*sqrt(P(2,2,:)))','g');
hold on
plot(t, x(:,2), '--g')

subplot(313)
shadedErrorBar(t,Xhat(:,3),squeeze(2*sqrt(P(3,3,:)))','r');
hold on
plot(t, x(:,3), '--r')

figure(2)
shadedErrorBar(t,Xhat(:,4),squeeze(2*sqrt(P(4,4,:)))','b');
hold on
shadedErrorBar(t,Xhat(:,5),squeeze(2*sqrt(P(5,5,:)))','g');
shadedErrorBar(t,Xhat(:,6),squeeze(2*sqrt(P(6,6,:)))','r');
plot(t, x(:,4:6), '--')
