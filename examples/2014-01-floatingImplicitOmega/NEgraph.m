...
for j = 1 : length(time)
    ...
    bnet{j}      = bnet0;
    [h2, Sh2, R, avgTime2(j)] = build4DOFestim(index, y, t, dt, o0, do0, p0);
    if j == 1
        [h, Sh, bnet{j}, R, avgTime(j), engine] = build4DOFbnet(bnet{j}, index, y, t, dt, o0, do0, p0, j);
    else
        [h, Sh, bnet{j}, R, avgTime(j), engine] = build4DOFbnet(bnet{j}, index, y, t, dt, o0, do0, p0, j, engine);
    end       
