...
for i = 1 : length(sens.parts) 
   ...
   if strcmp(sens_label(end-2:end), 'imu') || strcmp(sens_label(end-2:end), 'acc')
      
      ny = ny + 1;
      dy = sens.ndof{i};
      ymodel.labels{ny,1} = sens.labels{i};
      for j = 1 : dmodel.NB
         ymodel.Y{ny,j}   = zeros(dy,7);
         ymodel.Ys{ny,j}  = sparse(zeros(dy,7));
      ...
      if strcmp(ymodel.labels{ny,1}(end-2:end), 'imu')
         ymodel.Y{ny,link_ind}      = [[zeros(3,3) eye(3); eye(3) zeros(3,3)] zeros(dy, 1)];
         ymodel.Ys{ny,link_ind}     = sparse(1:6,[4:6 1:3],ones(dy,1), dy, 7);
      elseif strcmp(ymodel.labels{ny,1}(end-2:end), 'acc')
         ymodel.Y{ny,link_ind}      = [zeros(dy,3) eye(dy) zeros(dy, 1)];
         ymodel.Ys{ny,link_ind}     = sparse(1:3,4:6,ones(dy,1), dy, 7);
      end
...
for i = 1 : dmodel.NB
   ...
   for j = 1 : dmodel.NB
      ymodel.Y{ymodel.ny,j}   = zeros(1,7);
      ymodel.Ys{ymodel.ny,j}  = sparse(zeros(1,7));
   end
   ymodel.Y{ymodel.ny,i}  = [zeros(1,6) eye(1,1)];
   ymodel.Ys{ymodel.ny,i} = sparse(1,7,ones(1,1), 1, 7);
end
