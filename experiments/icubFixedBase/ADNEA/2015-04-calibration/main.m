...
if ~exist('preprocess.mat', 'file')
   ...
   data.parts       = {'inertial'                                 , 'left_arm_accelerometers', 'left_foot_inertial'        , 'left_hand_inertial'                  , 'right_arm_accelerometers', 'right_foot_inertial'       , 'right_hand_inertial'                 , 'torso_accelerometers'                    , 'head'      , 'left_arm'  , 'right_arm' , 'left_leg'  , 'right_leg' , 'torso'     };
   data.labels      = {'imu'                                      , 'la_acc'                 , 'lf_acc'                    , 'lh_acc'                              , 'ra_acc'                  , 'rf_acc'                    , 'rh_acc'                              , 'to_acc'                                  , 'h'         , 'la'        , 'ra'        , 'll'        , 'rl'        , 'to'        };
   data.ndof        = {12                                         , 18                       , 3                           , 6                                     , 18                        , 3                           , 6                                     , 12                                        ,  6          , 16          , 16          , 6           , 6           , 3           };
   data.index       = {'4:9'                                      , '1:3'                    , '1:3'                       , '1:3'                                 , '1:3'                     , '1:3'                       , '1:3'                                 , '1:3'                                     ,  '1:6'      , '1:16'      , '1:16'      , '1:6'       , '1:6'       , '1:3'       };
   ...
   data.visualize   = {1*data.plot                                , 1*data.plot              , 1*data.plot                 , 1*data.plot                           , 1*data.plot               , 1*data.plot                 , 1*data.plot                           , 1*data.plot                               , 1*data.plot , 1*data.plot , 1*data.plot , 1*data.plot , 1*data.plot , 1*data.plot };
   %%strucutre for urdf
   sens.parts       = {'chest+torso+neck_1+neck_2+head+imu_frame' , 'l_upper_arm+l_arm'      , 'l_upper_foot+l_foot+l_sole', 'l_forearm+l_wrist_1+l_hand+l_gripper', 'r_upper_arm+r_arm'       , 'r_upper_foot+r_foot+r_sole', 'r_forearm+r_wrist_1+r_hand+r_gripper', 'chest+torso+neck_1+neck_2+head+imu_frame'};
   sens.labels      = {'imu'                                      , 'la_acc'                 , 'lf_acc'                    , 'lh_acc'                              , 'ra_acc'                  , 'rf_acc'                    , 'rh_acc'                              , 'to_acc'                                  };
   sens.ndof        = {6                                          , 3                        , 3                           , 3                                     , 3                         , 3                           , 3                                     , 3                                         };
   sens.type        = {''                                         , 'analog:o'               , 'analog:o'                  , 'analog:o'                            , 'analog:o'                , 'analog:o'                  , 'analog:o'                            , 'analog:o'                                };
   sens.transform   = {'X_chest_imu'                              , 'X_l_upper_arm_la_acc'   , 'X_l_upper_foot_lf_acc'     , 'X_l_forearm_lh_imu'                  , 'X_r_upper_arm_ra_acc'    , 'X_r_upper_foot_rf_acc'     , 'X_r_forearm_rh_imu'                  , 'X_chest_to_acc'                          };
   ...
   for i = 1 : length(data.time)
      ...
      myDANEA = myDANEA.setState(data.q(:,i), data.dq(:,i));
      d_red   = zeros(7*dmodel.NB,1);
      for j = 1:dmodel.NB
         d_red(1+(j-1)*7 : 6+(j-1)*7, 1 ) = myRNEA.d( 1+(j-1)*26 :  6+(j-1)*26, 1 );
         d_red(7+(j-1)*7 : 7+(j-1)*7, 1 ) = myRNEA.d(26+(j-1)*26 : 26+(j-1)*26, 1 );
      end

      y(:,i) = myDANEA.simY(d_red, data.q(:,i), data.dq(:,i));
      if mod(i-1,100) == 0
...
data.y  = [];
ind_d2q = [];
for i = 1 : myDANEA.IDsens.sensorsParams.ny
   sens_str = myDANEA.IDsens.sensorsParams.labels{i};
   ...
   elseif length(sens_str)==9 && strcmp(sens_str, 'y_omega24')
      ...
      eval(['data.y  = [data.y ; data.ys_' myDANEA.IDsens.sensorsParams.labels{i} '];']);
   end
end
data.y(ind_d2q, :) = data.d2q;

data.Sy = [];
for i = 1 : length(myDANEA.IDsens.sensorsParams.labels)
   data.Sy = [data.Sy; diag(myDANEA.IDsens.sensorsParams.Sy{i})];
end
...
for l = 1 : length(label_to_plot)
   for k = 1 : myDANEA.IDsens.sensorsParams.ny
      if strcmp(myDANEA.IDsens.sensorsParams.labels{k}, label_to_plot{l}) || ...
            (strcmp(myDANEA.IDsens.sensorsParams.labels{k}, 'y_omega13') && strcmp(label_to_plot{l}, 'lh_gyr')) || ...
            (strcmp(myDANEA.IDsens.sensorsParams.labels{k}, 'y_omega24') && strcmp(label_to_plot{l}, 'rh_gyr'))
         figure
