...
dmodel   = autoTree(NB, 1.5, 1.5, 0.9);
...
if isempty(find(triu(P*[D; Y]*Q, 1), 1))
...
if isempty(find(tril(P*[D; Y]*Q, -1), 1))
   disp('[LU_RNEA] Factorisation with UMFPACK resulted in a lower trinagular matrix')
   res = 0;
else
   disp('[LU_RNEA] Error in performing the LU factorisation with UMFPACK!!')
   res = 1;
   return
end
