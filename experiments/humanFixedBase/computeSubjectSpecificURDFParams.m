...
for subjectID = 1:length(subjectList)
    for trialID = 1:length(trialList)
        ...
        f_GPWA_s = mean(temp.f_GPWA_s(1:pSelec,:));
       ...
       footAlpha = max([norm(P_G_rhee - P_G_lhee),norm(P_G_rtoe - P_G_ltoe)])*1e-3; 
       footBeta =  max([norm(P_G_lankle - P_G_lhee),norm(P_G_rankle - P_G_rhee)])*1e-3; 
       footGamma = max([norm(P_G_ltoe - P_G_lhee),norm(P_G_rtoe - P_G_rhee)])*1e-3;
       ...
       legBeta =  max(norm(P_G_lankle - P_G_lhip),norm(P_G_rankle - P_G_rhip))*1e-3;
       legGamma = 0.5*(0.5*(norm(P_G_rtoe-P_G_rhee) + norm(P_G_ltoe-P_G_lhee)))*1e-3;
       
       legV = legAlpha*legBeta*legGamma;
       
       torsoAlpha = 0.5*(norm(P_G_lhip - P_G_rhip)+norm(P_G_lsho - P_G_rsho))*1e-3;
       torsoBeta =  max(norm(P_G_lhip - P_G_lsho),norm(P_G_rhip - P_G_rsho))*1e-3;
       torsoGamma = legGamma;
       
       torsoV = torsoAlpha*torsoBeta*torsoGamma;
       
       totalV = footV+torsoV+legV;
       Totalm = norm(f_GPWA_s(:,1:3)) ./ 9.81;
       ...
       subjectParams(subjectID,trialID).footBeta = footAlpha;
       subjectParams(subjectID,trialID).footGamma = footAlpha;
...
save('subjectSizeParams.mat','subjectParams');
