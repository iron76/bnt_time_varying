...
%% BOWING TASK - EXPERIMENT2 - 
% Standard configuration (frontal view):
%
%             (C)
%              |               ^ x
%              |               |
%              |               |
%   (B)-------(A)        y <---o  z (exits, origin in A)
    ...
    % assumption: (C-A) coincides with x axes of imu
    x_G_imu = P_G_imuC - P_G_imuA;
    x_G_imu = x_G_imu ./ norm(x_G_imu); %norm
    
    % since (A-B) may not coincide with y axes of imu (assumption) 
    y_G_imuCandidate = P_G_imuB - P_G_imuA ;
