function [ sigma_ygivend, Sigma_EM] = computeEM_test(map, q, dq , Y, y, b_y, sigma_ygivend, varargin)
%% EM
...
for n = 1 : options.MAX_ITER
    ...
    for i = 1:len    
        ...
        A1 = 2*Y{i}*(Sigma_dgiveny+(mu_dgiveny*mu_dgiveny'))*Y{i}'-diag(diag(Y{i}*(Sigma_dgiveny+(mu_dgiveny*mu_dgiveny'))*Y{i}'));
        A2 = 2*yHat*(yHat') -diag(diag(yHat*yHat'));
        A3  = -(2*Y{i}*mu_dgiveny*yHat') - (2*yHat*mu_dgiveny'*Y{i}') + (2*diag(diag(Y{i}*mu_dgiveny*yHat')));
...
% % % %% LogLikelihood_joint
% % % 
% % % logL_EM_joint = zeros(options.MAX_ITER,1);
% % % for k =1:options.MAX_ITER
% % % 
% % % 
% % %     logL = 0;
% % %     for i = 1:len 
% % %         
% % %         [mu_dgiveny, ~, map] = computeMAP(map,q(:,i),dq(:,i),Y{i}, y(:,i),b_y(:,i), Sigma_EM{k});
% % %         yHat = y(:,i) - b_y(:,i);
% % %         % compute LogL
% % %         logL = logL + 0.5*( -(52+26)*log(2*pi) + log(det(full(inv(map.sigmaBarD)))) + log(det(Sigma_EM{k})) - (mu_dgiveny-map.muBarD)'*full(inv(map.sigmaBarD))*(mu_dgiveny-map.muBarD) - (yHat-(Y{i}*mu_dgiveny))'*Sigma_EM{k}*(yHat-(Y{i}*mu_dgiveny)));
% % %     end
% % %     logL_EM_joint(k,1)= 0.5*logL;
% % %     
% % % end
