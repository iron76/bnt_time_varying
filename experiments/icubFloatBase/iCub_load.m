...
robotName = 'icubFloat_fixedHeadWrist';
...
dmodel.appearance{i+1} = '1';

if(strcmp(robotName(1:9), 'icubFloat'))
   dmodel = floatbase(dmodel);
   for i = 1 : dmodel.NB
      if i < 7
         dmodel.linkname{i}   = 'base_link';
         dmodel.jointname{i}  = 'base_joint';
      else
         dmodel.linkname{i}   = R.body(i-5).linkname;
         dmodel.jointname{i}  = R.body(i-5).jointname;
      end
      dmodel.jtype{i}      = 'R';
   end
end
dmodel.jtype{1}      = 'Px';  dmodel.jtype{2}      = 'Py';   dmodel.jtype{3}      = 'Pz';
dmodel.jtype{4}      = 'Rx';  dmodel.jtype{5}      = 'Ry';   dmodel.jtype{6}      = 'Rz';
