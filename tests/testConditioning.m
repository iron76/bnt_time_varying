...
A = (D'*S_Dinv*D + S_dinv + Y'*S_Yinv*Y);
disp(['[CONDITIONING] cond(A) is ' num2str(condest(A), '%10.5e\n')]);
disp(['[CONDITIONING] Precision in the computations will be: 1e', num2str(round(log10(eps)+log10(condest(A))))])
% B = diag(1./max(abs(A')))*A;
