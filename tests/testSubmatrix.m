...
if (norm(A(I,J)-A(1:2, 1:3))) || (norm(As(1,1)-A(I,J)))
   disp('[SUBMATRIX] Something wrong with getting A(1,1)');
   res = 1;
end

[I, J] = indeces(As, 1, 2);
if (norm(A(I,J)-A(1:2, 4:10))) || (norm(As(1,2)-A(I,J)))
   disp('[SUBMATRIX] Something wrong with getting A(1,2)');
   res = 1;
end
...
if norm(As(3,2)-zeros(1,7))
   disp('[SUBMATRIX] Something wrong with setting A(3,2)');
   res = 1;
end

if norm(As(1:2,1)-[As(1,1); As(2,1)])
   disp('[SUBMATRIX] Something wrong with reading A(1:2,1)');
   res = 1;
end

if norm(As(2,[1 2])-[As(2,1) As(2,2)])
   disp('[SUBMATRIX] Something wrong with reading A(2,[1 2])');
   res = 1;
end

if norm(As([1 3],2)-[As(1,2); As(3,2)])
   disp('[SUBMATRIX] Something wrong with reading A([1 3],2)');
   res = 1;
end
...
if norm(As([1 3],2)-B)
   disp('[SUBMATRIX] Something wrong with setting A([1 3],2)');
   res = 1;
end

Bs = As([3 1], [1 2]);
if norm(As([3 1],[1 2])-Bs)
   disp('[SUBMATRIX] Something wrong with copying A([3 1],[1 2])');
   res = 1;
end
