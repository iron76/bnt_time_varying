function [mu, Sigma, B] = clg_Mstep_modified(w, Y, YY, YTY, X, XX, XY, varargin)
...
if isempty(X) % no regression
  ...
  [mu, Sigma] = mixgauss_Mstep_modified(w, Y, YY, YTY, varargin{:});
  return;
end
...
if cov_type(1)=='s'
  if ~tied_cov
    ...
    for i=1:Q
      % eqn 18
      A = [eye(length(Y(:,:,i))) zeros(size(mu(:,i)))];
