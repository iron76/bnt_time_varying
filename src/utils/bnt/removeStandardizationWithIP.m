...
for i = 1 : length(muStd)
   ...
   if(~isempty(i_prts))
      ...
      if( ~isa(bnet.CPD{i}, 'gaussian_net_wrench_CPD') )
        ...
        if nargin == 5
            if (~isempty(find(i==i_cov_learn,1)))
                bnet.CPD{i} = gaussian_CPD(bnet, i, 'twist', twist , 'force_weights', ones(6,1), 'acceleration_weights', ones(6,1), 'inertial_params', inertial_params, 'cov', SY^(-1)*SY_X*SY^(-1)','cov_prior_weight', covPriorWeight);
            else
                bnet.CPD{i} = gaussian_CPD(bnet, i, 'twist', twist , 'force_weights', ones(6,1), 'acceleration_weights', ones(6,1), 'inertial_params', inertial_params, 'cov',  SY^(-1)*SY_X*SY^(-1)',  'inertial_params', inertial_params, 'clamp_cov', 1);
            end
        elseif nargin == 3
            bnet.CPD{i} = gaussian_CPD(bnet, i, 'twist', twist , 'force_weights', ones(6,1), 'acceleration_weights', ones(6,1), 'inertial_params', inertial_params,  'cov',  SY^(-1)*SY_X*SY^(-1)');
        end   
      end
   else
      if( isa(bnet.CPD{i}, 'gaussian_net_wrench_CPD') )
