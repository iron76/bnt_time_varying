function [ model ] = autoTreeStochasticANEA( model , sModel, sUknown )
...
%% Sv
iSv_s = zeros(1*model.NB,1);
jSv_s = zeros(1*model.NB,1);
for i = 1 : model.NB
   [~, Sq    ] = jcalc( model.jtype{i} , 0);
   [~, jn(i) ] = size(Sq);
   iSv_s((i-1)*1+1 : 1*i, 1) = [6 ]';
   jSv_s((i-1)*1+1 : 1*i, 1) = [6 ]';
end
...
for i = 1 : model.NB
   ...
   model.Sv_inv = set(model.Sv_inv, inv(S), (i-1)*1+1, (i-1)*1+1);
   model.Sv     = set(model.Sv    ,     S , (i-1)*1+1, (i-1)*1+1);
end

%% Sw
iSw_s = zeros(1*model.NB,1);
jSw_s = zeros(1*model.NB,1);
for i = 1 : model.NB
   iSw_s((i-1)*1+1 : 1*i, 1) = [jn(i)]';
   jSw_s((i-1)*1+1 : 1*i, 1) = [jn(i)]';
end
...
for i = 1 : model.NB
   ...
   model.Sw_inv = set(model.Sw_inv, inv(S), (i-1)*1+1, (i-1)*1+1);
   model.Sw     = set(model.Sw    ,     S , (i-1)*1+1, (i-1)*1+1);
end
