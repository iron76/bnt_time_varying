...
Sw_inv = sparse(...
   [obj.IDmodel.modelParams.Sw_inv.is; obj.Sx_inv.is + 1*NB], ...
   [obj.IDmodel.modelParams.Sw_inv.js; obj.Sx_inv.js + 1*NB], ...
...
for i = 1:NB
   iSd(       (i-1)*1+1 :        1*i, 1) = 6;
   iSd(1*NB + (i-1)*1+1 : 1*NB + 1*i, 1) = obj.IDmodel.jn(i);
end
obj.Sd_sm = submatrix(iSd, iSd, obj.Sd);
for i = 1 : NB
   S_ind((i-1)*2+1:(i-1)*2+1, 1) =        (i-1)*1+1:       i*1;
   S_ind((i-1)*2+2:(i-1)*2+2, 1) = 1*NB + (i-1)*1+1:1*NB + i*1;
end
